...
function Grenade:initialize(world, parent, camera, x, y, vx, vy, kind)
  Entity.initialize(self, world, x, y, width, height)
  self.parent = parent
  self.camera = camera
  self.vx, self.vy  = vx, vy
  self.lived = 0
  self.ignoresParent = kind or true
end

function Grenade:filter(other)
  local kind = other.class.name
  if kind == 'Block'
  or(kind == 'Player' and not self.ignoresParent == kind)
...
function Grenade:moveColliding(dt)
  ...
  for i=1, len do
    local col = cols[1]
    if col.other.class.name == 'Player' and not self.ignoresParent == col.other.class.name then
      self:destroy()
      return
    end
